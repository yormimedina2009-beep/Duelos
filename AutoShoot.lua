...
local Config = {
    SilentAim = true,
    AutoShoot = true,
    FOV = 150,           -- Qué tan lejos del centro busca enemigos
    HitboxPart = "Head", -- "Head" o "HumanoidRootPart"
    VisibleCheck = true, -- No dispara a través de paredes
    Prediction = true,   -- Predice el movimiento del enemigo
    AutoFireDelay = 0.1, -- Segundos entre disparos
    AimSmoothness = 1,   -- 1 = instantáneo, mayor = más suave
    ShowFOV = true,
...
local function predecirPosicion(part, velocidadBala)
    ...
    -- Predice dónde estará en X segundos
...
RunService.RenderStepped:Connect(function()
    if Config.ShowFOV then
        ...
        fovCircle.Radius = Config.FOV
        fovCircle.Color = currentTarget and Color3.fromRGB(255, 50, 50) or Config.ESPColor
    else
        fovCircle.Visible = false
    end
end)
...
-- Componentes
local function crearToggle(texto, key, callback, y)
    ...
    circle.Position = Config[key] and UDim2.new(1, -18, 0.5, -8) or UDim2.new(0, 2, 0.5, -8)
    ...
    sw.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(circle, TweenInfo.new(0.2), {
            Position = a and UDim2.new(1, -18, 0.5, -8) or UDim2.new(0, 2, 0.5, -8)
        }):Play()
        if callback then callback(a) end
    end)
end

local function crearSlider(texto, key, min, max, callback, y)
    local f = Instance.new("Frame")
...
-- Crear elementos
crearToggle("Silent Aim", "SilentAim", nil, 48)
crearToggle("Auto Shoot", "AutoShoot", nil, 88)
crearToggle("Predicción de movimiento", "Prediction", nil, 128)
crearToggle("Verificar visibilidad", "VisibleCheck", nil, 168)
crearToggle("Mostrar círculo FOV", "ShowFOV", nil, 208)
crearSlider("FOV", "FOV", 30, 400, nil, 250)

local infoLabel = Instance.new("TextLabel")
infoLabel.Size = UDim2.new(1, -20, 0, 40)
infoLabel.Position = UDim2.new(0, 10, 1, -46)
infoLabel.BackgroundTransparency = 1
infoLabel.Text = "Objetivo: Ninguno"
...
pcall(function()
    StarterGui:SetCore("SendNotification", {
        Title = "🎯 Auto Shoot",
        Text = "Cargado correctamente. ¡A darle!",
        Duration = 4
    })
end)

print("✅ Kiritto Auto Shoot cargado")
